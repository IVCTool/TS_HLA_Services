...
2) Edit the   IVCTconfig.xml   file to set the absolute path of the folders of   sutDir   and   testSuites   these can remain where they were extracted from the zip files or be moved to another folder of the file system as desired.
...
3.1) Add the new test suite folder to the   IVCT_TS_HOME   folder.

3.2) Edit the   IVCTtestSuites\IVCTtestsuites.xml   and add the testSuite name, packageName and tsRunFolder for the new test suite.

The   name   is the folder name of the test suite.
...
3.3) Create the new   TcParam.json   to describe the test case parameters required for the new test suite. A sample file should be supplied by the test suite developer with each test suite release. The meaning of the parameters in this file must be well documented as to purpose and value range.
